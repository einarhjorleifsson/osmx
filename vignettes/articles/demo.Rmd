...
res <- sm_munge(cruises, stillingar)
```
...
```{r}
res |> sm_luw_plot(species = 1)
```

### Lengths-gutted

```{r}
res |> sm_lgw_plot(species = 1)
```

### Lengths vs gutted / ungutted

```{r}
res |> sm_l_gvu_plot(species = 1)
```

### Lengths vs liver / ungutted

```{r}
res |> sm_l_lvu_plot(species = 1)
```

### Lengths vs gonads / ungutted

```{r}
res |> sm_l_gonadsvu_plot(species = 1)
```
